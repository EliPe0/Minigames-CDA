...
export default function Home() {
    ...
    async function fetchStats() {
      ...
      try {
        ...
        if (error) {
          ...
          setStats({ 
            scores: data[0].total_scores, 
            attempts: data[0].total_attempts 
          });
        }
      } catch (err) {
  ...
  const nextSlide = () => setCurrentIndex((prev) => (prev + 1) % MINIGAMES_LIST.length);
  const prevSlide = () => setCurrentIndex((prev) => (prev - 1 + MINIGAMES_LIST.length) % MINIGAMES_LIST.length);

  const activeModule = MINIGAMES_LIST[currentIndex];
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 w-full max-w-4xl mx-auto z-10 animate-page-reveal">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-[#0c0c0c] border border-neutral-800/60 p-5 rounded-2xl flex items-center gap-4 shadow-sm hover:border-neutral-700 transition-colors">
            <div className="p-3 bg-neutral-900 rounded-xl border border-neutral-800 text-neutral-400">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="15" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
            </div>
            <div>
              <div className="text-[10px] text-neutral-500 uppercase tracking-widest font-black">Pontuações Registradas</div>
              <div className="text-xl font-bold text-neutral-200 mt-0.5">{loadingStats ? <span className="animate-pulse opacity-50">...</span> : stats.scores}</div>
            </div>
          </div>
          ...
          <div className="bg-[#0c0c0c] border border-neutral-800/60 p-5 rounded-2xl flex items-center gap-4 shadow-sm hover:border-neutral-700 transition-colors">
            ...
            <div>
              ...
              <div className="text-xl font-bold text-neutral-200 mt-0.5">{loadingStats ? <span className="animate-pulse opacity-50">...</span> : stats.attempts}</div>
            </div>
          </div>
          ...
          <div className="bg-[#0c0c0c] border border-neutral-800/60 p-5 rounded-2xl flex items-center gap-4 shadow-sm hover:border-neutral-700 transition-colors">
            ...
            <div>
              ...
              <div className="text-xl font-bold text-neutral-200 mt-0.5">{MINIGAMES_LIST.length}</div>
            </div>
          </div>
        </div>

        <div className="w-full flex items-center justify-between mb-4 border-b border-neutral-900 pb-2">
          <h2 className="text-xs font-black uppercase tracking-widest text-neutral-400">Selecione um Módulo</h2>
          <div className="text-[10px] text-neutral-600 uppercase tracking-widest font-bold">Módulo {currentIndex + 1} de {MINIGAMES_LIST.length}</div>
        </div>

        <div className="relative w-full flex items-center justify-center gap-4 sm:gap-6">
          <button onClick={prevSlide} className="cursor-pointer hidden sm:flex p-4 bg-[#0c0c0c] border border-neutral-800 rounded-2xl text-neutral-500 hover:text-white hover:border-neutral-600 active:scale-95 transition-all shadow-xl shrink-0"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg></button>
          ...
          <div key={currentIndex} className="w-full max-w-xl bg-[#0c0c0c] border border-neutral-800 rounded-3xl overflow-hidden flex flex-col shadow-2xl animate-page-reveal">
            <div className="w-full h-48 sm:h-56 bg-neutral-950 relative overflow-hidden border-b border-neutral-800 transition-colors p-2">
              <div className="w-full h-full rounded-2xl overflow-hidden relative">
                <img src={activeModule.gif} alt={`Preview ${activeModule.title}`} className="w-full h-full object-cover" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
            ...
            <div className="p-6 sm:p-8 flex flex-col justify-between">
              <div className="mb-8">
                <h3 className={`text-lg font-black uppercase tracking-wider mb-4 transition-colors ${activeModule.colorClass}`}>{activeModule.title}</h3>
                <div className="text-neutral-400 text-xs sm:text-[13px] leading-relaxed text-left font-sans whitespace-pre-wrap">
                  {activeModule.objective}
                </div>
              </div>
              <button onClick={() => navigate(activeModule.path)} className={`cursor-pointer w-full py-4 rounded-xl font-black text-xs sm:text-sm uppercase tracking-widest transition-all duration-300 transform active:scale-[0.98] font-mono ${activeModule.btnClass}`}>Iniciar Módulo</button>
            </div>
          </div>

          <button onClick={nextSlide} className="cursor-pointer hidden sm:flex p-4 bg-[#0c0c0c] border border-neutral-800 rounded-2xl text-neutral-500 hover:text-white hover:border-neutral-600 active:scale-95 transition-all shadow-xl shrink-0"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between w-full mt-8 max-w-xl gap-6">
          ...
          <div className="flex justify-center gap-2 w-full sm:w-auto">
            {MINIGAMES_LIST.map((_, idx) => (
              <button key={idx} onClick={() => setCurrentIndex(idx)} className={`cursor-pointer h-1.5 rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-8 bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'w-2 sm:w-3 bg-neutral-800 hover:bg-neutral-700'}`} />
            ))}
          </div>
        </div>
      </div>

      {/* OVERLAY ABOUT MANTIDO IDENTICO */}
          ...
          <div className="max-w-xl w-[95%] max-h-[90vh] bg-[#0c0c0c] border border-neutral-800 rounded-2xl shadow-2xl relative overflow-hidden animate-elastic-pop flex flex-col font-mono cursor-default">
            <div className="h-14 bg-[#141414] flex items-center justify-between px-6 border-b border-neutral-800/60 shrink-0">
              <div className="flex items-center gap-2 text-amber-500 text-sm font-black tracking-widest uppercase">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-[0_0_6px_rgba(245,158,11,0.6)]"><path d="M20 16V7a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v9m16 0H4m16 0 1.28 2.55a1 1 0 0 1-.9 1.45H3.62a1 1 0 0 1-.9-1.45L4 16"></path></svg>
                Info do Sistema
              </div>
              <button onClick={() => setShowAbout(false)} className="cursor-pointer text-neutral-500 hover:text-white transition-colors p-2">✕</button>
            </div>
            <div className="p-6 sm:p-8 flex flex-col gap-5 overflow-y-auto">
              <h2 className="text-xl sm:text-2xl font-black text-red-500 tracking-widest uppercase border-b border-neutral-900 pb-4 mb-1">Leia com atenção</h2>
              ...
              <div className="bg-[#050505] border border-neutral-800 p-4 sm:p-5 rounded-xl flex flex-col gap-4 mt-2 shadow-inner">
                <div className="flex justify-between items-center text-xs font-bold tracking-widest uppercase">
                  ...
                  <a href="https://discord.com/users/432954675734511626" target="_blank" rel="noopener noreferrer" className="cursor-pointer text-purple-400 hover:text-purple-300 hover:underline transition-all duration-200">EliPe0</a>
                </div>
                <div className="flex justify-between items-center text-xs font-bold tracking-widest uppercase border-t border-neutral-900/60 pt-4">
                  ...
                  <a href="https://github.com/EliPe0/Minigames-CDA" target="_blank" rel="noopener noreferrer" className="cursor-pointer text-blue-400 hover:text-blue-300 hover:underline transition-all duration-200">Acessar GitHub</a>
                </div>
                <div className="flex justify-between items-center text-xs font-bold tracking-widest uppercase border-t border-neutral-900/60 pt-4">
                  <span className="text-neutral-500">Site do Servidor:</span>
                  <a href="https://cidadealta.gg/" target="_blank" rel="noopener noreferrer" className="cursor-pointer text-amber-500 hover:text-amber-300 hover:underline transition-all duration-200">cidadealta.gg</a>
                </div>
              </div>
            </div>
            <div className="p-4 sm:p-5 border-t border-neutral-900/60 bg-[#101010] flex justify-end shrink-0">
              <button onClick={() => setShowAbout(false)} className="cursor-pointer px-8 py-3 bg-neutral-200 hover:bg-white active:scale-[0.97] text-black font-mono font-black text-[11px] sm:text-xs uppercase tracking-widest rounded-xl transition-all shadow-md">Retornar ao Dashboard</button>
            </div>
          </div>
        </div>,
        document.body
