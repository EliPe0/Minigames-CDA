    ...
    <div className="flex flex-col items-center justify-center flex-1 bg-[#050505] p-6 font-sans select-none w-full relative overflow-hidden text-white animate-page-reveal">
      ...
      {!showHint && (
        <button
          ...
          className="cursor-pointer absolute top-6 right-6 z-40 flex items-center gap-2 px-4 py-2 bg-[#0c0c0c] border border-neutral-800 rounded-xl text-neutral-400 hover:text-blue-400 hover:border-blue-500/40 transition-all font-mono text-[12px] font-bold uppercase tracking-wider"
        >
        ...
        <div className={`w-full max-w-[540px] bg-[#0c0c0c] border rounded-2xl shadow-2xl flex flex-col relative overflow-hidden border-neutral-800 transition-transform ${
          ...
          <div className="flex items-center justify-between border-t border-neutral-900 p-5 w-full relative transition-colors gap-6 bg-neutral-900/10 z-20">
            ...
            <div className="flex-shrink-0">
              {gameState === 'playing' ? (
                <button onClick={pararSistema} className="px-6 py-2.5 cursor-pointer bg-red-600 hover:bg-red-500 text-white font-mono font-black text-xs uppercase tracking-widest rounded-xl transition-all active:scale-95">
                  Abortar Sistema
                </button>
              ) : gameState === 'won' || gameState === 'lost' ? (
                <button onClick={pararSistema} className="px-6 py-2.5 cursor-pointer bg-neutral-800 hover:bg-neutral-700 text-white font-mono font-black text-xs uppercase tracking-widest rounded-xl transition-all active:scale-95">
                  Voltar ao Menu
                </button>
              ) : (
                <button onClick={iniciarSistema} className="px-8 py-2.5 cursor-pointer bg-amber-500 hover:bg-amber-400 text-black font-mono font-black text-xs uppercase tracking-widest rounded-xl transition-all active:scale-95">
                  Iniciar Sistema
                </button>
              )}
            </div>
      ...
      <div className={`fixed top-0 right-0 h-full w-[340px] bg-[#0c0c0c] border-l border-neutral-800 z-50 flex flex-col font-mono shadow-2xl transition-transform duration-300 ease-in-out ${showHint ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-14 bg-[#141414] border-b border-neutral-800/60 flex items-center justify-between px-5 text-blue-500 text-[11px] font-black uppercase tracking-wider shrink-0">
          <div className="flex items-center gap-1.5">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#0e69df" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
              <circle cx="12" cy="12" r="10"></circle>
              <line x1="12" y1="16" x2="12" y2="12"></line>
              <line x1="12" y1="8" x2="12.01" y2="8"></line>
            </svg>
            Painel de Dica: Porta Malas
          </div>
