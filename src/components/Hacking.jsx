...
export default function Hacking() {
  ...
  const getGroupFromPos = (pos, count = 4) => {
    let group = [pos];
    for (let i = 1; i < count; i++) group.push((pos + i) >= 80 ? (pos + i) - 80 : pos + i);
    return group;
  };
  ...
  const moveCodes = useCallback(() => setCodesPos(prev => (prev + 1) % 80), []);
  ...
  const start = useCallback(() => {
    setCodesPos(0); setCurrentPos(43); setIsTargetHidden(false); setProgress(100);
    ...
    let generatedCodes = [];
    for (let i = 0; i < 80; i++) generatedCodes.push(randomSetChar(activeSet) + randomSetChar(activeSet));
    ...
    timerProgressInterval.current = setInterval(() => {
      ...
      setProgress((restante / TEMPO_TOTAL_MS) * 100);
      setTimerDisplay((restante / 1000).toFixed(2));
      if (restante <= 0) checkRef.current();
    }, 30);

    if (Math.random() > 0.75) { 
      timerHideInterval.current = setInterval(() => setIsTargetHidden(prev => !prev), 2000); 
    }
  }, [moveCodes]);
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if ((gameState === 'idle' || gameState === 'won' || gameState === 'lost') && key_pressed === 'Enter') {
      e.preventDefault(); reset(true); return;
    }

    if (gameState === 'playing' && valid_keys.includes(key_pressed)) {
      e.preventDefault();
      if (key_pressed === 'Enter') { checkRef.current(); return; }
      setCurrentPos(prev => {
        let next = prev;
        switch (key_pressed) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#050505] p-4 font-sans select-none w-full relative overflow-hidden text-white animate-page-reveal">
        ...
        <div className="w-full max-w-[640px] bg-[#0c0c0c] border border-neutral-800 rounded-2xl shadow-2xl flex flex-col relative overflow-hidden">
          ...
          <div className="h-11 bg-[#141414] flex items-center justify-center px-6 border-b border-neutral-800/40 text-neutral-400 text-xs font-mono font-black uppercase tracking-wider transition-colors">
            <div className="flex items-center gap-2">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2.5"><rect x="2" y="4" width="20" height="14" rx="2" ry="2"></rect><line x1="1" y1="20" x2="23" y2="20"></line></svg>
          ...
          <div className="flex flex-col w-full pb-6 pt-5 relative">
            <div className="flex flex-col items-center gap-2 px-8 mb-6">
              <div className="flex justify-between w-full text-[10px] font-mono font-black text-neutral-400 uppercase tracking-widest px-0.5">
                <span className="flex items-center gap-1.5"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={timerBarColor} strokeWidth="2.5" className="transition-colors duration-200"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg> Tempo Restante</span>
            ...
            <div className="flex gap-3 items-center justify-center h-20 mb-4 w-full px-8">
              {gameState === 'playing' ? (!isTargetHidden ? targetBlock.map((char, i) => <div key={i} className="bg-purple-950/20 text-purple-400 font-mono font-black text-3xl px-4 py-1.5 rounded-xl border border-purple-500/30 min-w-[64px] text-center shadow-md animate-page-reveal">{char}</div>) : <span className="text-red-500 border border-red-500/20 bg-red-500/5 px-4 py-1.5 rounded-xl text-xs font-mono font-black tracking-widest uppercase animate-pulse">SINAL INTERROMPIDO</span>) : <div className="text-neutral-500 font-mono text-[10px] font-black tracking-widest uppercase py-3 transition-colors">{gameState === 'idle' ? 'AGUARDANDO LIBERAÇÃO DO TERMINAL' : gameState === 'won' ? 'CRIPTOGRAFIA QUEBRADA' : 'SINAL CORROMPIDO'}</div>}
            </div>
              ...
              {gameState === 'playing' || gameState === 'won' || gameState === 'lost' ? (
                <div className={`grid grid-cols-10 gap-x-2 gap-y-3 w-[500px] mx-auto font-mono font-bold text-base ${isMirroredActive ? 'mirrored-soup' : ''}`}>
                  {displayCodes.map((code, idx) => {
                    ...
                    if (gameState === 'playing' && isSelected) cellColorClass = "text-purple-400 font-black scale-110 opacity-100 z-10 drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]";
                    else if (gameState === 'won' && isSelected) cellColorClass = "text-emerald-400 font-black opacity-100";
                    else if (gameState === 'lost' && idx === ((correctPosRef.current - codesPos + 80) % 80)) cellColorClass = "text-emerald-400 font-black opacity-100"; 
                    return <div key={idx} className={`text-center h-6 transition-all ${cellColorClass}`}>{code}</div>;
                  })}
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center font-mono text-center gap-3 z-30 transition-colors"><div className="p-3 bg-neutral-900 border border-neutral-800 rounded-2xl"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-purple-400"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg></div></div>
              )}

              <GameOverOverlay gameState={gameState} lostText="CRÍTICO | TENTATIVA DETECTADA E BLOQUEADA" wonText="SUCESSO | SINAL ENCONTRADO E CONSOLIDADO" />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between border-t border-neutral-900 p-5 w-full relative transition-colors gap-6 bg-neutral-900/10 z-20">
            <div className="flex flex-wrap gap-x-4 gap-y-2 items-center flex-1 text-[10px] font-bold tracking-wider uppercase text-neutral-500 font-mono">
              {gameState === 'playing' ? (
                <>
                  <div className="flex items-center gap-1.5"><kbd className="bg-[#141414] border border-neutral-800 text-neutral-200 px-1.5 py-0.5 rounded text-[9px] font-black font-mono">W</kbd><kbd className="bg-[#141414] border border-neutral-800 text-neutral-200 px-1.5 py-0.5 rounded text-[9px] font-black font-mono">A</kbd><kbd className="bg-[#141414] border border-neutral-800 text-neutral-200 px-1.5 py-0.5 rounded text-[9px] font-black font-mono">S</kbd><kbd className="bg-[#141414] border border-neutral-800 text-neutral-200 px-1.5 py-0.5 rounded text-[9px] font-black font-mono">D</kbd><span className="text-neutral-500">Mover Cursor</span></div>
                  <div className="flex items-center gap-1.5"><kbd className="bg-[#141414] border border-neutral-800 text-neutral-200 px-2 py-0.5 rounded text-[9px] font-black font-mono">ENTER</kbd><span className="text-neutral-500">Validar Sinal</span></div>
                </>
              ) : (
            ...
            <div className="flex-shrink-0">
              {gameState === 'playing' ? <button onClick={() => reset(false)} className="cursor-pointer px-6 py-2.5 bg-red-600 hover:bg-red-500 text-white font-mono font-black text-xs uppercase tracking-widest rounded-xl transition-all active:scale-95">Abortar Sistema</button> : gameState === 'won' || gameState === 'lost' ? <button onClick={() => reset(false)} className="cursor-pointer px-6 py-2.5 bg-neutral-800 hover:bg-neutral-700 text-white font-mono font-black text-xs uppercase tracking-widest rounded-xl transition-all active:scale-95">Voltar ao Hub</button> : <button onClick={() => reset(true)} className="cursor-pointer px-8 py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-mono font-black text-xs uppercase tracking-widest rounded-xl transition-all active:scale-95 shadow-md shadow-purple-500/10">Iniciar Sistema</button>}
            </div>
          </div>
      ...
      >
        <p className="text-justify animate-elastic-pop">O terminal exibirá uma sequência fixa de 4 blocos roxos no topo da tela. Seu objetivo consiste em rastrear exatamente esse bloco idêntico camuflado dentro do fluxo em movimento abaixo.</p>
        <p className="text-justify animate-elastic-pop">Utilize os controles <span className="text-white font-bold">W, A, S, D</span> ou as setas direcionais para mover o cursor quadrado. A velocidade de rotação do código exige foco rápido.</p>
        <p className="text-justify animate-elastic-pop">Assim que o seu quadrado roxo de seleção estiver perfeitamente posicionado cobrindo toda a sequência correta, aperte <span className="text-purple-400 font-bold font-black transition-colors">ENTER</span> imediatamente para consolidar o bypass. Qualquer clique fora da posição corrompe o sinal e reseta o streak.</p>
      </HintPanel>
